...
public class InstitucionEducativaDAO extends InstitucionEducativaAbstractDAO implements IInstitucionEducativa {
	...
	public ResultSet ejecutarSentenciaCarreras(Statement statement) {
		...
		try {
			return statement.executeQuery(
					"SELECT C.ID, C.NOMBRE, C.TITULO FROM CARRERAS C INNER JOIN CARRERAS_INSTITUCION CI INNER JOIN INSTITUCIONES I WHERE C.ID = CI.ID_CARRERA AND CI.ID_INSTITUCION = I.ID AND I.NRO = "
							+ nro);
	...
	public ResultSet ejecutarSentenciaDocentes(Statement statement) {
		...
		try {
			return statement.executeQuery(
					"SELECT D.ID, D.NOMBRE, D.APELLIDO, D.LEGAJO FROM DOCENTES D INNER JOIN DOCENTES_INSTITUCION DI INNER JOIN INSTITUCIONES I WHERE D.ID = DI.ID_DOCENTE AND DI.ID_INSTITUCION = I.ID AND I.NRO = "
							+ nro);
		} catch (SQLException e) {
			e.printStackTrace();
			Archivo archivo = new Archivo();
